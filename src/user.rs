...
impl Into<[u8; 32]> for AuthDataSalt {
    fn into(self) -> [u8; 32] {
        [
            ...
            self.a16, self.a17, self.a18, self.a19,
            self.a20, self.a21, self.a22, self.a23,
            self.a24, self.a25, self.a26, self.a27,
            self.a28, self.a29, self.a30, self.a31,
        ]
    }
}
...
impl MainPassword {
    ...
    pub fn plain(&self, ik_or_main: &String) -> Result<Vec<u8>, UserOperationError> {
        ...
        let intermediate_derived_key = crate::derive_key(&intermediate_key.as_str(), temp.as_slice());
