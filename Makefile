...
target/$(TARGET)/$(BUILD_TYPE)/pam_polyauth-mount: target/$(TARGET)/$(BUILD_TYPE)/libpam_polyauth.so
	cargo build --frozen --offline --all-features --$(BUILD_TYPE) --bin pam_polyauth-mount --target=$(TARGET) --target-dir target
